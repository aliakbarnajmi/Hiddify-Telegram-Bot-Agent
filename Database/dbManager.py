...
# --------------- Server ---------------------------
    ...
    def add_server(self, url, user_path, user_limit, title=None, description=None, status=True, default_server=False):
        cur = self.conn.cursor()
        try:
            cur.execute(
                "INSERT INTO servers(url,user_path,title,description,user_limit,status,default_server) VALUES(?,?,?,?,?,?,?)",
                (url, user_path, title, description, user_limit, status, default_server))
            self.conn.commit()
